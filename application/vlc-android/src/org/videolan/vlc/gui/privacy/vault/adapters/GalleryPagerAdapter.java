...
import org.videolan.vlc.gui.privacy.vault.adapters.viewholders.GalleryPagerViewHolder;
import org.videolan.vlc.gui.privacy.vault.data.FileType;
import org.videolan.vlc.gui.privacy.vault.data.GalleryFile;
import org.videolan.vlc.gui.privacy.vault.encryption.Encryption;
import org.videolan.vlc.gui.privacy.vault.encryption.MyDataSourceFactory;
...
public class GalleryPagerAdapter extends RecyclerView.Adapter<GalleryPagerViewHolder> {
    ...
    private void setupDirectoryView(@NonNull GalleryPagerViewHolder holder, FragmentActivity context, GalleryFile galleryFile) {
        ...
        if (firstFile != null) {
            Glide.with(context).load(firstFile.getThumbUri()).into(((GalleryPagerViewHolder.GalleryPagerDirectoryViewHolder) holder).binding.thumb);
        }
    }
        ...
        if (!payloads.isEmpty()) {
            for (Object o : payloads) {
                if (o instanceof Boolean) {
                    ...
                    GalleryGridAdapter.Payload p = (GalleryGridAdapter.Payload) o;
    ...
    private void setupVideoView(GalleryPagerViewHolder.GalleryPagerVideoViewHolder holder, FragmentActivity context, GalleryFile galleryFile) {
        ...
        Glide.with(context).load(galleryFile.getThumbUri()).into(holder.binding.imgThumb);
    ...
    private void playVideo(FragmentActivity context, Uri fileUri, GalleryPagerViewHolder.GalleryPagerVideoViewHolder holder) {
        ...
        if (player == null) {
            ...
            player = new ExoPlayer.Builder(context).setMediaSourceFactory(progressiveFactory).build();
            player.setRepeatMode(Player.REPEAT_MODE_ONE);
            players.put(pos, player);
        }
        MediaItem mediaItem = new MediaItem.Builder().setMimeType("video/*").setUri(fileUri).build();
    ...
    private void loadImage(Uri outputUri, GalleryPagerViewHolder holder, FragmentActivity context) {
        if (holder instanceof GalleryPagerViewHolder.GalleryPagerImageViewHolder) {
            ...
            Glide.with(context).asGif().load(outputUri).into(((GalleryPagerViewHolder.GalleryPagerGifViewHolder) holder).binding.gifImageView);
        }
    }
    ...
    private void showButtons(GalleryPagerViewHolder holder, boolean show) {
        if (isFullscreen) {
            ...
            holder.parentBinding.getRoot().setBackgroundColor(MaterialColors.getColor(weakReference.get(), R.attr.backgroundColor, Color.WHITE));
        }
        if (show) {
            holder.parentBinding.lLButtons.setVisibility(View.VISIBLE);
            holder.parentBinding.txtName.setVisibility(View.VISIBLE);
    ...
    private void showExport(FragmentActivity context, GalleryFile galleryFile) {
        Dialogs.showConfirmationDialog(context, context.getString(R.string.dialog_export_title), context.getString(R.string.dialog_export_message), (dialog, which) -> new Thread(() -> {
    ...
    private void shareOrOpenWith(FragmentActivity context, Uri decryptedCacheUri, boolean open) {
        ...
        if (uri != null) {
            Intent intent;
            if (open) {
                intent = new Intent(Intent.ACTION_VIEW, uri).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            } else {
                intent = new Intent().setAction(Intent.ACTION_SEND).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION).setDataAndType(uri, context.getContentResolver().getType(uri)).putExtra(Intent.EXTRA_STREAM, uri);
            }
            ...
            if (galleryFile.getNote() != null) {
                ...
                View.OnClickListener onClickListener = v -> {
                    if (expanded[0]) {
                        ...
                        holder.parentBinding.note.setText(context.getString(R.string.gallery_note_click_to_show));
                        holder.parentBinding.noteLayout.setBackgroundColor(MaterialColors.getColor(context, R.attr.backgroundColor, Color.BLACK));
                    } else {
    ...
    public void onViewDetachedFromWindow(@NonNull GalleryPagerViewHolder holder) {
        if (holder instanceof GalleryPagerViewHolder.GalleryPagerVideoViewHolder) {
            GalleryPagerViewHolder.GalleryPagerVideoViewHolder vh = (GalleryPagerViewHolder.GalleryPagerVideoViewHolder) holder;
            pauseVideo(vh);
        }
    ...
    public void onViewRecycled(@NonNull GalleryPagerViewHolder holder) {
        if (holder instanceof GalleryPagerViewHolder.GalleryPagerImageViewHolder) {
            GalleryPagerViewHolder.GalleryPagerImageViewHolder h = (GalleryPagerViewHolder.GalleryPagerImageViewHolder) holder;
            h.binding.imageView.recycle();
        } else if (holder instanceof GalleryPagerViewHolder.GalleryPagerVideoViewHolder) {
            GalleryPagerViewHolder.GalleryPagerVideoViewHolder vh = (GalleryPagerViewHolder.GalleryPagerVideoViewHolder) holder;
            releaseVideo(vh);
        }
