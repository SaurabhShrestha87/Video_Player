...
object VLCDownloadManager : BroadcastReceiver(), DefaultLifecycleObserver {
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        intent?.run {
            ...
            ExternalSubRepository.getInstance(context).getDownloadingSubtitle(id)
                ?.let { subtitleItem ->
                    val (state, localUri) = getDownloadState(id)
                    when (state) {
                        DownloadManager.STATUS_SUCCESSFUL -> dlDeferred?.complete(
                            SubDlSuccess(
                                id, subtitleItem, localUri
                            )
                        )
    ...
    override fun onStart(owner: LifecycleOwner) {
        AppContextProvider.appContext.applicationContext.registerReceiverCompat(
            this, IntentFilter(DownloadManager.ACTION_DOWNLOAD_COMPLETE), true
        )
    }
    ...
    suspend fun download(context: FragmentActivity, subtitleItem: SubtitleItem) {
        ...
        ExternalSubRepository.getInstance(context.applicationContext)
            .addDownloadingItem(id, subtitleItem)
        when (val result = deferred.await()) {
            is SubDlFailure -> downloadFailed(result.id, context)
            is SubDlSuccess -> downloadSuccessful(
                result.id, result.subtitleItem, result.localUri, context
            )
        }
    }
            ...
            downloadedPaths.forEach {
                if (Extensions.SUBTITLES.contains(".${it.split('.').last()}")) {
                    ExternalSubRepository.getInstance(context).saveDownloadedSubtitle(
                        idSubtitle, it, mediaUri.path!!, subLanguageID, movieReleaseName
                    )
                } else Toast.makeText(
                    context, R.string.subtitles_download_failed, Toast.LENGTH_SHORT
                ).show()
            }
    ...
    private suspend fun getFinalDirectory(
        ...
        val folder = subtitleItem.mediaUri.path.getParentFolder()
            ?: return context.getExternalFilesDir("subs")?.absolutePath
        val canWrite = context.isStarted() && context.getExtWritePermission(folder.toUri())
        return if (canWrite) folder
        else (context.applicationContext.getExternalFilesDir(null))?.absolutePath
            ?: defaultSubsDirectory
    }
    ...
    private fun getDownloadPath(subtitleItem: SubtitleItem) =
        "VLC/${subtitleItem.movieReleaseName}_${subtitleItem.idSubtitle}.zip"
    ...
    private fun getDownloadState(downloadId: Long): Pair<Int, String> {
        ...
        val status = if (statusIndex != -1) cursor.getInt(statusIndex)
        else DownloadManager.STATUS_FAILED

        val localUriIndex = cursor.getColumnIndex(DownloadManager.COLUMN_LOCAL_URI)
        val localUri = if (localUriIndex != -1) cursor.getString(localUriIndex)
        else ""
...
private class SubDlSuccess(val id: Long, val subtitleItem: SubtitleItem, val localUri: String) :
    SubDlResult()
