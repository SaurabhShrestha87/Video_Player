...
class EqualizerNewFragment : BottomSheetDialogFragment(), Slider.OnChangeListener {
    ...
    private var selectedPresetPos = 0;
    ...
    private fun fillViews() = lifecycleScope.launch(start = CoroutineStart.UNDISPATCHED) {
        ...
        binding.equalizerSave.setOnClickListener { createSaveCustomSetDialog(selectedPresetPos, displayedByUser = true, onPause = false) }
        ...
        // presets
        adapter = ArrayAdapter(requireContext(), android.R.layout.simple_list_item_1, allSets)
        ...
        binding.chipGroup.post {
            // Add each item in the adapter as a Chip
    ...
    private inner class BandListener(private val index: Int) : OnEqualizerBarChangeListener {
        ...
        override fun onProgressChanged(value: Float, fromUser: Boolean) {
            ...
            if (getEqualizerType(pos) == TYPE_PRESET) {
                ...
                updateAlreadyHandled = true
                setPresetSelection(presetCount + customCount)
    ...
    fun setPresetSelection(pos: Int) {
        binding.chipGroup.check(binding.chipGroup[pos].id)
    }
